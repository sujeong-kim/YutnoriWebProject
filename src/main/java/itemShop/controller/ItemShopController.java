...
public class ItemShopController {
	...
	@RequestMapping(value="/itemShop/shoppingCartSuc.do")
	public ModelAndView shoppingCartSuc(HttpServletRequest request) {
		int item_charges = Integer.parseInt(request.getParameter("item_charges"));
		String item_codes = request.getParameter("item_codes");
		item_codes = item_codes.substring(1);
		String[] arr = item_codes.split(":");
		...
		for(int i=0;i<arr.length;i++) {
			item_code = arr[i];
			insertCount = userService.insertHistory(userDTO.getUser_id(), item_code);
			cartOut = itemShopService.deleteCart(userDTO.getUser_id(), item_code);
		}

		
		
		int suc = itemShopService.itemPaymentSuccess(item_charges, userDTO.getUser_id());
		int user_cash = userDTO.getUser_cash();
		
		
		if(suc>0 && insertCount>0 && cartOut > 0) {	// 결제 완료되었고 장바구니에서 삭제완료			
			userDTO.setUser_cash(user_cash - item_charges);
			session.setAttribute("session_dto", userDTO);			
		}
		...
		modelAndView.addObject("insertCount", insertCount);
		modelAndView.addObject("payOK", "ok");
		modelAndView.addObject("item_charge", item_charges);
		modelAndView.addObject("user_cash", user_cash);
		modelAndView.setViewName("../itemShop/itemPayment.jsp");
