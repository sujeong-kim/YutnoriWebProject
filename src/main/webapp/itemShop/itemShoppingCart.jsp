...
<script type="text/javascript">
	$(function(){
			...
			if($("input[type=checkbox]#warning_check").is(":checked")){
				...
				if((${session_dto.user_cash} - Number(charge)) < 0 ){
					...
					if($("input[type=checkbox].check").is(":checked")){
						// 결제하기 눌렀을 때  아이템이 하나라도 체크된 경우 
						// item_code값 가져오기
						var values = document.getElementsByName("checkCart");
						var item_codes="";
						for(var i=0;i<values.length;i++){
							if(values[i].checked){
								item_codes= item_codes + ":" + values[i].value ;
							}
						}
						// 결제화면으로 이동하기
						location.href="../itemShop/shoppingCartSuc.do?item_codes="+item_codes+"&item_charges="+charge;
					}else{
						alert("하나이상의 아이템을 선택해주세요.");
					}
		...
		$("input[type=checkbox].check").click(function(){
			if($(this).is(":checked")){
				charge = String(Number(charge)+Number($(this).attr("id")));
...
<c:forEach var="itemShopDTO" items="${list }">
	<tr id="content">
		<td><input type="checkbox" class="check" name="checkCart" value="${itemShopDTO.item_code }" id="${itemShopDTO.item_charge }"></td>
...
<div class="total_charge">
	<label style="color: gray;">보유금액 : ${session_dto.user_cash}원</label>
</div>
<div class="total_charge">
	<label>결제금액 : </label><label id="charge">0</label><label>원</label>
</div>
<div id="warning">
